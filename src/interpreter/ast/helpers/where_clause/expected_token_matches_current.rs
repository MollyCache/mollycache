use crate::interpreter::ast::{LogicalOperator, Parentheses, WhereStackElement, parser::Parser};
...
pub fn next_expected_token_from_current(
    expected_token: &WhereClauseExpectedNextToken,
    where_stack_element: &WhereStackElement,
    parser: &mut Parser,
) -> Result<WhereClauseExpectedNextToken, String> {
    match where_stack_element {
        ...
        WhereStackElement::LogicalOperator(logical_operator) => match logical_operator {
            LogicalOperator::Not => {
                if *expected_token != WhereClauseExpectedNextToken::ConditionLeftParenNot {
                    return Err(parser.format_error_nearby());
                }
        ...
        WhereStackElement::Parentheses(parentheses) => match parentheses {
            Parentheses::Left => {
                if *expected_token != WhereClauseExpectedNextToken::ConditionLeftParenNot {
                    return Err(parser.format_error_nearby());
                }
...
mod tests {
    use super::*;
    use crate::interpreter::ast::{Operand, Operator, Value, WhereCondition};

    #[test]
    fn handles_next_condition_being_condition() {
        let mut parser = Parser::new(vec![]);
        let where_stack_element = WhereStackElement::Condition(WhereCondition {
        ...
        assert!(
            next_expected_token_from_current(&expected_token, &where_stack_element, &mut parser)
                .is_err()
        );
