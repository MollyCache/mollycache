...
impl<'a> Scanner<'a> {
    ...
    fn read_identifier(&mut self, start: usize) -> TokenTypes {
        ...
        return match &self.input[start..=self.current] {
            ...
            slice if slice.eq_ignore_ascii_case("VARCHAR") => TokenTypes::Varchar,
            slice if slice.eq_ignore_ascii_case("CHAR") => TokenTypes::Char,
            slice if slice.eq_ignore_ascii_case("FLOAT") => TokenTypes::Float,
            slice if slice.eq_ignore_ascii_case("BOOLEAN") => TokenTypes::Boolean,
            slice if slice.eq_ignore_ascii_case("TIMESTAMP") => TokenTypes::Timestamp,
