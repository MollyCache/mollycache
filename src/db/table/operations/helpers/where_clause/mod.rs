...
pub fn row_matches_where_stack(
    table: &Table,
    row: &Row,
...
mod tests {
    ...
    fn matches_where_stack_works_short_circuits() {
        ...
        let mut spy_where_condition_evaluator = SpyWhereConditionEvaluator {
            conditions_evaluated: vec![],
        };
        let row = Row(vec![Value::Integer(1), Value::Text("John".to_string())]);
        let condition_1 = WhereCondition {
            l_side: Operand::Identifier("id".to_string()),
            operator: Operator::Equals,
            r_side: Operand::Value(Value::Integer(1)),
        };
        let condition_2 = WhereCondition {
            l_side: Operand::Identifier("id".to_string()),
            operator: Operator::Equals,
        ...
        let result = where_stack::matches_where_stack(
            &table,
            &row,
            &where_stack,
            &mut spy_where_condition_evaluator,
        );
        assert!(result.is_ok() && result.unwrap());
        assert_eq!(
            spy_where_condition_evaluator.conditions_evaluated,
            vec![condition_1]
        );
    }
