...
fn parse_in_range(s: &str, name: &str, min: i64, max: i64, default: i64) -> Result<i64, String> {
    ...
    if !(min..=max).contains(&value) {
        return Err(format!("{} out of range ({}-{}): {}", name, min, max, value));
    }
    Ok(value)
}
...
pub fn parse_timevalue(time_value: &Value) -> Result<f64, String> {
    match time_value {
        Value::Text(text) if text == "now" => {
            ...
            let jdn = (unix_ms as f64 / 86400000.0) + 2440587.5;
            return Ok(jdn);
        }
        Value::Text(txt) => {
            ...
            let jdn_int = day + (153 * m + 2) / 5 + 365 * y + y / 4 - y / 100 + y / 400 - 32045;
            
            let time_fraction = (hour as f64) / 24.0 
                + (minute as f64) / 1440.0 
                + (second as f64 + frac_seconds) / 86400.0;
            
            let mut jdn = (jdn_int as f64) + time_fraction - 0.5;
            ...
            if !timezone_part.is_empty() {
                ...
                let tz_minute = parse_in_range(timezone_minute, "timezone minute", 0, 59, 0)? as f64;
...
mod tests {
    ...
    fn test_parse_timevalue() {
        ...
        assert_eq!(parse_timevalue(&Value::Text("2025-12-12".to_string())), Ok(2461021.5));
        ...
        assert!((result - 2461022.020833333).abs() < 0.000001);
        assert_eq!(parse_timevalue(&Value::Text("2025-12-12T12:00:00".to_string())), Ok(2461022.0));
