...
impl<'a> Condition<'a> {
    fn evaluate(
        ...
        table: &Table,
        row: &Row,
        where_clause_evaluator: &mut dyn MatchesWhereClause,
    ) -> Result<bool, String> {
        match self {
            Condition::Boolean(boolean) => Ok(*boolean),
            Condition::WhereCondition(where_condition) => {
                where_clause_evaluator.matches_where_clause(table, row, where_condition)
            }
        }
    }
}

pub fn matches_where_stack(
    table: &Table,
    row: &Row,
    where_stack: &Vec<WhereStackElement>,
    where_clause_evaluator: &mut dyn MatchesWhereClause,
) -> Result<bool, String> {
        ...
        match where_stack_element {
            ...
            WhereStackElement::LogicalOperator(logical_operator) => {
                let pop1 = match result_stack.pop() {
                    ...
                    None => {
                        return Err(format!(
                            "Error evaluating where clause with table: {:?}",
                            table
                        ));
                    }
                };
                match logical_operator {
                    LogicalOperator::Not => {
                        result_stack.push(Condition::Boolean(!pop1.evaluate(
                            table,
                            row,
                    ...
                    LogicalOperator::And => {
                        ...
                        result_stack.push(Condition::Boolean(
                            pop2.evaluate(table, row, where_clause_evaluator)?
                                && pop1.evaluate(table, row, where_clause_evaluator)?,
                        ));
                    }
                    LogicalOperator::Or => {
                        let pop2 = match result_stack.pop() {
...
mod tests {
    ...
    fn matches_where_stack_returns_true_if_row_matches_where_stack_with_single_condition() {
        ...
        let where_stack = vec![WhereStackElement::Condition(WhereCondition {
            l_side: Operand::Identifier("id".to_string()),
            operator: Operator::Equals,
            r_side: Operand::Value(Value::Integer(1)),
        })];
        let result =
            matches_where_stack(&table, &row, &where_stack, &mut WhereConditionEvaluator {});
