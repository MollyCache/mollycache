...
pub fn insert(table: &mut Table, statement: InsertIntoStatement) -> Result<(), String> {
    ...
    if let Some(statement_columns) = &statement.columns {
        ...
        for row in statement.values {
            let row_values = validate_and_clone_row(table, &row)?;
...
mod tests {
    ...
    fn insert_into_table_is_generated_correctly() {
        ...
        let expected = vec![vec![Value::Integer(1), Value::Text("John".to_string()), Value::Integer(25), Value::Real(1000.0)]];
    ...
    fn insert_into_table_with_columns_is_generated_correctly() {
        ...
        assert!(insert(&mut table, statement).is_ok());
        let expected = vec![
            vec![Value::Integer(1), Value::Text("John".to_string()), Value::Null, Value::Null],
            vec![Value::Integer(2), Value::Text("Jane".to_string()), Value::Null, Value::Null],
        ];
        assert_eq!(table.rows, expected);
    }
}